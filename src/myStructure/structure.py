...
__all__ = ('myStructureData', 'myKind', 'Site')
...
class myStructureData(StructureData):
    ...
    def append_kind(self, kind):
        ...
        new_kind = myKind(kind=kind)  # So we make a copy

        if kind.name in [k.name for k in self.mykinds]:
    ...
    def mykinds(self):
        ...
        return [myKind(raw=i) for i in raw_kinds]
